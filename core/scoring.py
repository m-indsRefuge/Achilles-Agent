...
class RetrievalScorer:
    ...
    def score(self, chunk: Dict, query_embedding: List[float], metadata: Optional[Dict] = None) -> Dict:
        ...
        # Note: raw_similarity is expected to be in [-1, 1] for cosine,
        # so we clamp it to [0, 1].
        ...
        feedback_score = success_score / (success_score + self.feedback_k)

        # 3. Recency Component (Exponential Decay: exp(-lambda * age))
        current_time = time.time()
        last_time_str = chunk.get('last_accessed') or chunk.get('created_at')
